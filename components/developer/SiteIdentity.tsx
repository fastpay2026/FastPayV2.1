      ...
      <form onSubmit={handleSaveConfig} className="bg-[#0f172a] p-12 rounded-[4rem] border border-white/5 shadow-2xl space-y-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase mr-4">{t('services_title')}</label><input value={tempConfig.servicesTitle} onChange={e => setTempConfig({ ...tempConfig, servicesTitle: e.target.value })} className="w-full p-5 bg-black/40 border border-white/10 rounded-2xl font-black outline-none focus:border-sky-500 transition-all" /></div>
            <div className="space-y-2"><label className="text-[10px] font-black text-slate-500 uppercase mr-4">{t('services_subtitle')}</label><textarea value={tempConfig.servicesSubtitle} onChange={e => setTempConfig({ ...tempConfig, servicesSubtitle: e.target.value })} className="w-full p-5 bg-black/40 border border-white/10 rounded-2xl font-black outline-none h-32 focus:border-sky-500 transition-all" /></div>
          </div>
        </div>
