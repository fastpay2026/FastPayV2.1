    ...
    <div className={`flex flex-col min-h-screen bg-[#0a0a0a] text-white overflow-x-hidden ${isRtl ? 'text-right' : 'text-left'}`} dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <nav className="fixed w-full z-[90] top-6 px-4 md:px-12 lg:px-24">
        <div className="max-w-[1600px] mx-auto bg-white/5 backdrop-blur-3xl border border-white/10 rounded-2xl md:rounded-[4rem] py-2 md:py-3 px-4 md:px-8 flex justify-between items-center shadow-[0_40px_100px_rgba(0,0,0,0.5)] gap-4">
          ...
          <div className="hidden xl:flex gap-12 2xl:gap-16 text-slate-200 font-black text-[18px] 2xl:text-[20px] uppercase tracking-widest justify-center items-center flex-1 overflow-x-auto no-scrollbar px-2">
            {[
              { label: t(siteConfig.navHomeText || 'nav_home'), id: 'home', visible: !siteConfig.hideHeroSection },
              { label: t(siteConfig.navRaffleText || 'raffle'), id: 'raffle-ad', visible: !siteConfig.hideRaffleSection },
              { label: t(siteConfig.navGatewayText || 'nav_gateway'), id: 'gateway-ad', visible: !siteConfig.hideGatewaySection },
              { label: t(siteConfig.navTransferText || 'nav_transfer'), id: 'salary-ad', visible: !siteConfig.hideSalarySection },
              { label: t(siteConfig.navTradingText || 'nav_trading'), id: 'trading-ad', visible: !siteConfig.hideTradingSection }
      ...
      {currentPath === 'home' ? (
        <>
             ...
             <div className="max-w-[1800px] mx-auto group glass-card rounded-3xl md:rounded-[6rem] overflow-hidden flex flex-col lg:flex-row-reverse items-stretch min-h-[500px] md:min-h-[700px] shadow-3xl border border-violet-500/10 hover:border-violet-500/40 transition-all duration-1000 relative">
                <div className="w-full lg:w-[55%] p-10 md:p-32 space-y-8 md:space-y-12 flex flex-col justify-center relative z-10">
                   ...
                   <div className="flex flex-wrap gap-6 md:gap-8 items-center pt-4 md:pt-8">
                     <button onClick={onLoginClick} className="bg-violet-600 px-10 md:px-20 py-4 md:py-8 rounded-2xl md:rounded-[3rem] font-black text-lg md:text-2xl hover:bg-violet-500 hover:scale-105 transition-all shadow-[0_30px_60px_rgba(124,58,237,0.3)] w-full md:w-max">{t(siteConfig.gatewayCtaText || 'activate_gateway')}</button>
                  ...
                  <div className="w-full lg:w-[55%] p-10 md:p-32 space-y-8 md:space-y-12 flex flex-col justify-center relative z-10 bg-gradient-to-br from-slate-950 via-[#020617] to-indigo-950/20">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8 py-4 md:py-6">
                       <div className="p-6 md:p-8 bg-white/5 rounded-2xl md:rounded-3xl border border-white/5 space-y-4">
                          ...
                          <h4 className="text-lg md:text-xl font-black">{t(siteConfig.rafflePrize1Title || 'porsche_title')}</h4>
                          <p className="text-xs md:text-sm text-slate-500 font-bold">{t(siteConfig.rafflePrize1Desc || 'porsche_desc')}</p>
                       </div>
                       <div className="p-6 md:p-8 bg-white/5 rounded-2xl md:rounded-3xl border border-white/5 space-y-4">
                          <span className="text-4xl md:text-5xl">🕋</span>
                          <h4 className="text-lg md:text-xl font-black">{t(siteConfig.rafflePrize2Title || 'umrah_title')}</h4>
                          <p className="text-xs md:text-sm text-slate-500 font-bold">{t(siteConfig.rafflePrize2Desc || 'umrah_desc')}</p>
                       </div>
                    </div>

                    <div className="flex flex-wrap gap-6 md:gap-8 items-center pt-4 md:pt-8">
                      <button onClick={onRegisterClick} className="bg-amber-600 px-10 md:px-20 py-4 md:py-8 rounded-2xl md:rounded-[3rem] font-black text-lg md:text-2xl hover:bg-amber-500 hover:scale-105 transition-all shadow-[0_30px_60px_rgba(245,158,11,0.3)] w-full md:w-max">{t(siteConfig.raffleCtaText || 'book_ticket')} {siteConfig.raffleEntryCost}$</button>
          ...
          {(!siteConfig.hideSalarySection || !siteConfig.hideTradingSection) && (
            <section className="py-40 px-8 md:px-24 space-y-40 max-w-[1800px] mx-auto">
              {!siteConfig.hideSalarySection && (
                <div id="salary-ad" className="group glass-card rounded-3xl md:rounded-[6rem] overflow-hidden flex flex-col lg:flex-row items-stretch min-h-[500px] md:min-h-[700px] shadow-3xl border border-indigo-500/10 hover:border-indigo-500/40 transition-all duration-1000 relative">
                   <div className="w-full lg:w-[55%] p-10 md:p-32 space-y-8 md:space-y-12 flex flex-col justify-center relative z-10">
                      ...
                      <button onClick={onRegisterClick} className="bg-indigo-600 px-10 py-4 md:px-20 md:py-8 rounded-2xl md:rounded-[3rem] font-black text-lg md:text-2xl hover:bg-indigo-500 hover:scale-105 transition-all shadow-[0_30px_60px_rgba(79,70,229,0.3)] w-full md:w-max">{t(siteConfig.salaryCtaText || 'request_pre_financing')}</button>
                   </div>
                   <div className="w-full lg:w-[45%] h-64 md:h-auto relative bg-slate-900 overflow-hidden">
              ...
              {!siteConfig.hideTradingSection && (
                <div id="trading-ad" className="group glass-card rounded-3xl md:rounded-[6rem] overflow-hidden flex flex-col lg:flex-row items-stretch min-h-[500px] md:min-h-[700px] shadow-3xl hover:border-sky-500/30 transition-all duration-1000">
                   <div className="w-full lg:w-[55%] p-10 md:p-32 space-y-8 md:space-y-12 flex flex-col justify-center relative z-10">
                      ...
                      <button onClick={onLoginClick} className="bg-sky-600 px-10 py-4 md:px-20 md:py-8 rounded-2xl md:rounded-[3rem] font-black text-lg md:text-2xl hover:bg-sky-500 hover:scale-105 transition-all shadow-[0_30px_60px_rgba(14,165,233,0.4)] w-full md:w-max">{t(siteConfig.tradingCtaText || 'enter_pro_platform')}</button>
                   </div>
                   <div className="w-full lg:w-[45%] h-64 md:h-auto relative bg-slate-900 overflow-hidden">
          ...
          {!siteConfig.hideServicesSection && (
            <section id="services" className="py-20 md:py-60 px-6 md:px-24">
               <div className="max-w-[1600px] mx-auto">
                  <div className="text-center space-y-6 md:space-y-8 mb-20 md:mb-40">
                    ...
                    <h3 className="text-lg md:text-3xl text-slate-400 font-bold max-w-4xl mx-auto">{t(siteConfig.servicesSubtitle || 'services_subtitle')}</h3>
                  </div>
                 ...
                 <ul className="space-y-6 md:space-y-10 text-slate-400 font-bold text-lg md:text-2xl">
                    {pages.length > 0 ? (
                      pages.filter(p => 
                        p.isActive && 
                        p.showInFooter && 
                        p.slug !== 'security-standards' && 
                        !p.slug.toLowerCase().includes('security') &&
                        !p.title.toLowerCase().includes('security') &&
                        !p.title.includes('الأمان')
                      ).map(p => (
                ...
                <div className="max-w-none text-slate-300 font-medium leading-relaxed dynamic-content-render page-content-rich">
                  {(() => {
                    try {
                      ...
                      return <div className="rich-text-container" dangerouslySetInnerHTML={{ __html: langContent }} />;
                    } catch (e) {
                      return <div className="rich-text-container" dangerouslySetInnerHTML={{ __html: activeCustomPage.content }} />;
                    }
                  })()}
                </div>
              </div>
