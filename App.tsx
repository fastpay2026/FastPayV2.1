...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isSupabaseConfigured && !hasMigratedConfig.current) {
      ...
      const defaultMappings: { [key: string]: string } = {
        ...
        "بوابة FastFlow Group لإدارة الأصول والتداول الفوري وحماية الثروات الرقمية بأعلى معايير الأمان العالمية.": "hero_subtitle",
        ...
        "FastFlow Group هي المعيار العالمي للمدفوعات الرقمية عالية الأمان، نجمع بين التكنولوجيا المتطورة والخدمات المالية المتميزة.": "footer_about",
        ...
        "شارك الآن في سحب FastFlow الشهري للفوز بسيارة رياضية خارقة أحدث طراز، أو رحلة عمرة VIP.": "raffle_ad_desc",
        ...
        "بوابة دفع FastFlow": "gateway_ad_title",
  ...
  const syncUser = useCallback(async (user: User) => {
    if (isInitialLoad.current) {
      console.warn('App: Sync blocked: Initial load in progress.');
      return;
    }
    if (!user.id) {
      console.error('App: Sync blocked: Invalid user data (no ID).', user);
      return;
    }
